...
# Added for visualize supposedly
opt.objlist = [2]
...
iteration = 4
bs = 1
dataset_config_dir = 'datasets/exo/dataset_config'
output_result_dir = 'experiments/eval_result/exo'
...
testdataset = PoseDataset_exo('eval', num_points, False, opt.dataset_root, 0.0, True)
testdataloader = torch.utils.data.DataLoader(testdataset, batch_size=1, shuffle=False, num_workers=10)

# Added for visualizing results
#intrinsic_calibration = cam_data[first_scene]['cam_K']
...
meta = yaml.load(meta_file, Loader=SafeLoader)
for obj in objlist:
...
for i, data in enumerate(testdataloader, 0):
    ...
    #if sym_list != None:
    if sym_list != None and idx[0].item() in sym_list:
